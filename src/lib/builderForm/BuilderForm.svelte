<script lang="ts">
	...
	const handleRemoveIngredient = (ingredient: number) => {
		const filteredArray = ingredientList.filter(
			(item: string, index: number) => index !== ingredient
		);
	...
	const handleRemoveInstruction = (instruction: number) => {
		const filteredArray = instructionList.filter(
			(item: InstructionsInterface, index: number) => index !== instruction
		);
	...
	const onSubmit = (event: any) => {
		...
		// Transform from formdata into standard json object
		formData.forEach(function (value, key) {
			input[key] = value;
		});

		// Add the lists that are not saved in the form fields
...
<form class="builderForm" on:submit|preventDefault={onSubmit}>
		...
		{#if keywordList.length}
			<UnorderedList>
				{#each keywordList as keyword, i}
					<ListItem item={keyword}
						><Button formType="button" variant="link" on:click={() => handleRemoveKeyword(i)}
							>[remove]</Button
						></ListItem
					>
				{/each}
			...
			<UnorderedList>
				{#each ingredientList as ingredient, i}
					<ListItem item={ingredient}
						><Button formType="button" variant="link" on:click={() => handleRemoveIngredient(i)}
							>[remove]</Button
						></ListItem
					>
				{/each}
	...
	<div class="formGroup double">
		...
		/>
		<label class="formLabel" for="instructionStep"
			>Instruction for step {instructionList.length + 1}</label
		>
		<input
			class="formInput"
			id="instructionStep"
